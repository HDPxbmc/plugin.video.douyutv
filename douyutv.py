...
# helloworld.py
# hello world demo
import urllib,json,APIHelper,xbmcplugin,xbmcgui
import urlparse
base_url = sys.argv[0]
...
if(action is None):
	...
	for game in data:
		...
		listitem=xbmcgui.ListItem(label = game["game_name"],iconImage = game["game_src"],thumbnailImage = game["game_src"],path = url)
	...
	try:
		...
		for game in data:
			...
			listitem=xbmcgui.ListItem(label = game["room_name"],iconImage = game["room_src"],thumbnailImage = game["room_src"],path = url)
